...
- **Fixed — daily run data not reaching the Dashboard**: there are two separate "Done" buttons — the always-visible one in the run banner (`confirmDoneRun()`) and a "Done — Review & Submit" one that only appears once every needed flavor is made (`showRunSummary()` → `submitSummary()` → `writeRunSummary()`). Only the second path ever wrote a `run_completed` entry to `storeEvents` — the first, more prominent button just silently cleared the run's tracked data with no Firestore write at all, even though its own confirmation message ("N buckets made, tap Done again") explicitly acknowledges real production happened. `confirmDoneRun()` now calls `writeRunSummary()` before clearing state, same as the other path (`writeRunSummary()` already no-ops safely if nothing was made, so this is safe in every case).
- **Not fixed, flagged as a known lower-priority residual risk**: `writeRunSummary()` reads the in-memory `storeEvents` array, appends one entry, and writes the whole array back — two devices completing separate runs at the literal same moment could still have one overwrite the other's entry. Left as-is rather than restructuring `storeEvents` into per-event subdocuments, since that's a much larger change for a very low-probability race (a single store having two simultaneous run completions), but worth knowing about.
- **Note:** both fixes only stop the bad behavior *going forward*. If either bug already fired and saved bad data before this fix — Novelties defaults overwriting real targets, or a "Done"-via-banner run never getting recorded — that specific data needs to be re-entered once; after that, both will hold correctly.

### Fix: Flavor List Disappearing on Sign-Out / Restart / Another Device (2026-08-06) — High Priority
- **Fixed:** the root cause of the Ice Cream Run flavor list appearing to reset itself under normal use, with no user action. `loadRunForDate()` (`js/store-org.js`) already seeded a brand-new day's list from the store's persistent default (`currentFlavorList`) when the day's `runs/{date}` doc didn't exist yet — but only for the *initial* load. The live listener set up right after did not: its first callback fires immediately with the server's current state, which on any day nobody has saved to yet genuinely reports "doesn't exist," and that unseeded callback silently wiped the just-seeded list back to blank moments after it appeared. Every reported trigger — signing out and back in, closing and reopening the app, opening from a different device — works by re-running `loadRunForDate()` from scratch, so all three hit this exact race. Extracted the seeding into `_seedTodayRunData()`, now applied to both the initial load and every live update from the listener.
- **Note:** this only stops the list from being wiped *going forward*. If a save happened to fire while the list was already showing blank from this bug (e.g. someone tapped a target field during the brief empty flash), that would have written the empty state back into the persistent `currentFlavorList` default itself, in which case the list needs to be rebuilt once more after this fix — after that, it will hold.
...
### Multi-Device Field Testing Fixes: Persistence, Listener Recovery, Corporate Dashboard (2026-08-04)
- **Fixed:** the secondary Firebase Auth instance added for store-owner account provisioning (previous entry below) was set up *before* the core app's own Firebase wiring in `index.html`'s startup script — meaning a failure to initialize it, on any device, would have silently prevented the entire app's Firestore/Auth setup from ever running. Moved to run last and wrapped in try/catch, so it can no longer take anything else down with it. Not confirmed as an actual cause of any reported symptom, but a real single-point-of-failure regardless.
- **Fixed:** explicitly set Firebase Auth to `browserLocalPersistence` instead of relying on the SDK's own default (which may pick IndexedDB-backed storage in some environments). IndexedDB inside an iOS "Add to Home Screen" standalone PWA has a long history of WebKit-specific reliability issues with sessions not restoring after the app is fully closed and relaunched — reported as "continuously signs out, every time I open the app I have to sign back in" on an iPhone, even though the connection itself was confirmed working ("Loaded from cloud" showed correctly once signed in). localStorage-backed persistence is simpler and more battle-tested on WebKit; this app never needed IndexedDB's multi-tab sync benefit (one shared login per store, not multiple simultaneous tabs).
- **Fixed:** a genuine sign-out breaks every active Firestore listener (`permission-denied`, since the security rules require `isOrgMember()`) in a way a same-session token refresh never does — but only the Run tab's listener was unconditionally re-subscribed on every reload; Novelties/Inventory only (re)subscribed when their "working date" was unset, so a device that kept getting signed out (see above) would silently stop receiving Novelties updates from other devices after the first sign-out, even after signing back in. `index.html`'s `onAuthStateChanged` now flags a sign-out event; `loadAll()` (`js/store-org.js`) resets the Run/Novelties/Inventory working-date state — and re-establishes the Novelties listener — whenever that flag is set, not just on a store switch.
- **Fixed:** the corporate System Dashboard's "Active Flavors"/"Shortages" figures (store cards and the detail panel) have shown 0 for every store regardless of real state — they read `store.activeFlavors`, a field that hasn't actually existed on the store document since `activeFlavors` moved into the `runs/{date}` subcollection (so a store's full run history doesn't bloat the doc read on every app load). `renderMultiStoreOverview()` (`js/dashboard.js`) now fetches each store's today's run doc alongside the store list to hydrate this correctly.
- Investigated an empty Manager (store-level) Dashboard report: `writeRunSummary()` only writes a `run_completed` activity entry if at least one bucket was actually made that run (`if (_totalBucketsMade <= 0 && cateringMadeCount === 0) return;`) — Reset alone never writes one. If a device's testing was dipping/holding adjustments + Reset without completing a run via the Made-stepper + Done, an empty Dashboard is the correct, expected state, not a bug — worth confirming against an actual completed run.

### Corporate Account Build-Out: Regions, Store Provisioning (2026-08-04)
- Confirmed already working (no changes needed): corporate accounts already see every store in the org-wide store picker (Settings → Store Name) and switching stores already reloads every tab + the Manager Dashboard for the newly-selected store; the System Dashboard (`showCorporateDashboard()`) already aggregates every store; assigning/reassigning an existing account's store access already exists (Settings → Users & Roles → "Stores" editor); adding a new store already exists (corporate dashboard's "Add Another Store").
- **Added:** lightweight regions — `store.region` is a free-text tag (not a separate collection), settable when adding a store or edited inline from a store's card in the System Dashboard. `renderMultiStoreOverview()` now shows clickable region chips above the store grid — click one to scope the aggregate stats, store cards, and Top Flavors to just that region; "All Regions" resets. Hidden entirely until more than one region is actually in use, so nothing changes for orgs not using this yet.
- **Fixed:** `createOrgAndStore()`'s member-doc write used to overwrite the creator's `stores[]` with just the one new store id instead of appending to it — harmless for CORPORATE_ADMIN today (that role bypasses `stores[]` scoping entirely) but incorrect data to leave in Firestore. Now merges into whatever they already had.
- **Added:** Settings → Users & Roles → "Create Store Owner Account" (corporate-only) — provisions a brand-new login immediately, pre-assigned to whichever store(s) and role you pick, instead of the previous only option (the new owner self-serving on the entry screen, landing on whatever store happened to be selected on that device). No email is sent — the app has no backend to send one from — so the generated temporary password is shown once for corporate to relay themselves; the owner can change it later via "Forgot password?". Since the client SDK normally signs the *caller* into any account it creates, this runs against a new secondary Firebase App instance (`index.html`) used only for this, so creating the account never signs the corporate admin out of their own session.

### Real-Time Cross-Device Sync Fixes (2026-08-04)
- **Fixed:** the root cause of changes made on one device (a Reset, a target/dipping/holding edit, a Novelties Target/On Hand/Made change) appearing to "not stick" or go missing on other devices — even on a fresh sign-in. Every one of `saveAll()`, `saveNoveltiesCatalog()`, `saveNoveltiesLog()`, `saveInventoryCatalog()`, `saveInventoryLog()`, and `saveStoreSettings()` fired its own unawaited Firestore write on *every single field edit*, with no coordination between overlapping calls. Two writes to the same doc are two independent network requests that can complete in either order — an older-but-slower write (e.g. a dipping change from moments before, delayed by a weak connection) could finish *after* a newer one (e.g. Reset) and silently win, discarding the newer state on the server. New shared helper `_makeCoalescedSaver()` (`appHelpers.js`) fixes this: if a write is already in flight when the function is called again, it just flags "run once more after this one finishes" instead of firing a second overlapping write, and since the eventual re-run always reads whatever is currently in memory, the very last write is guaranteed to reflect the latest state no matter how many edits landed while a save was busy.
- **Added:** the Novelties tab's `noveltiesLog/{date}` now has a live `onSnapshot` listener (`js/novelties.js`), matching the Ice Cream Run tab's `runs/{date}` — previously it only loaded once when the tab opened, by design ("Novelties counting is typically single-device/single-session"), which is exactly why Target/On Hand/Made changes from another device never showed up without a full reload.
- **Fixed:** switching stores on one device (`selectStore()`) could leave the Novelties and Inventory tabs — and, in the case of a recalled past date, the Run tab — showing the *previous* store's already-loaded data, since their "which date is loaded" state only initializes once (`if (!_workingNoveltiesDate) ...`) rather than on every store load. `loadAll()` now resets that state (and unsubscribes the old Novelties listener) whenever the current store actually changes, without doing so on unrelated re-inits like an auth token refresh (which would otherwise have bounced a manager out of a recalled past date they were legitimately reviewing).
- Investigated the reported empty Dashboard: `showManagerDashboard()` reads directly from the same in-memory store data the fixes above keep correctly synced, so this should now resolve as a consequence — but if a Dashboard is empty because no run was ever marked *Done* that day (Reset alone doesn't write a `run_completed` event), that's accurate, not a bug. Worth re-confirming against a completed run.
