...
## In Progress (newest session — post-tab-bar cleanup pass)
- ✅ **Fixed: modals rendered behind the bottom tab bar.** `.modal-backdrop` (the Edit Flavors modal) had `z-index: 100`, below the tab bar's `z-index: 250` — its Done button and bottom content were visually covered. Bumped to `z-index: 260`. Every other overlay (`.store-overlay` 300, `.var-overlay`/`.summary-overlay` 400, `.cabinet-overlay` 500, `.entry-overlay` 900, `.pin-overlay` 950) was already above the tab bar, so this was the one real fix needed.
- ✅ **Flavor roster additions are now corporate-only and org-wide.** "Add a new flavor" in the Edit Flavors modal (`#addFlavorSection`) is hidden unless `CORPORATE_ADMIN` (`renderPicker()` in `js/roster.js`); `addNewToRoster()` now writes to `organizations/{orgId}.customFlavors` (new org-level field, populated by `loadOrgMetadata()` into `_orgCustomFlavors`) instead of the store doc, so one corporate addition appears in every store's roster (`applyData()` merges `MASTER_ROSTER + _orgCustomFlavors + store's legacy customAdded`). Non-corporate accounts can still toggle which existing roster flavors are on today's list, just can't add new ones.
- ✅ **Roster picker's trash icon repurposed**: previously showed on every flavor and permanently deleted it from the roster (`removeFromRoster`); now only shows on flavors already selected for today, and just unselects them (same action as tapping the name — `toggleFlavor`). `removeFromRoster`/`undoRosterDelete` are left defined but unused (no remaining caller) rather than ripped out mid-cleanup-pass.
- ✅ **Fixed: gray copyright symbol next to "Oreo".** `MASTER_ROSTER` entries used "Oreo®️" — the ® followed by U+FE0F (variation selector-16), which forces emoji-style presentation and made the symbol render in its own color regardless of the surrounding text's CSS color. Removed the trailing U+FE0F from all 5 affected entries (4 Oreo flavors + "Reese's Peanut Butter Cup®") — plain "®" now inherits the flavor's type color correctly.
- ✅ Sign Out button (`#authBtn`) hidden while the Store Settings tab is active (`switchTab()` in `js/app-core.js`) — Settings has its own account actions (Switch Store etc.), so a header Sign Out there was redundant clutter.
- ✅ Removed the standalone "☰ Edit Flavors" toolbar button from the Ice Cream Run tab — it's reachable via Store Settings → Manage Flavor Roster now, which is already PIN-gated by requiring the Settings tab in the first place, so no separate PIN check is needed on that entry point.
- Flavor Target-column editing was **already** gated by `_managerUnlocked` from the earlier PIN rework (unchanged code, but now correctly reflects the Firestore-backed PIN) — confirmed via test, no change needed.
- ✅ **Novelties page redesigned** to mirror the Ice Cream Run's flavor table: Item / Target / On Hand / To Make / Made columns (`js/novelties.js`, full rewrite of `renderNoveltiesPage()`). Target editing is PIN-gated exactly like the flavor Target column (editable input when `_managerUnlocked`, read-only + tap-to-unlock otherwise). Removed the ability to add/remove items from this page entirely (no more per-category "Add Item" input or trash icons — catalog is a fixed preset now). "Made" button prompts for a quantity (`prompt()`, consistent with the app's existing native-dialog conventions) and adds it to that item's on-hand count; dropped the old boolean "done"/checked-off/undo-toast model since it's no longer part of the design.

## In Progress (prior session — login/PIN rework)
